...
class ResNetFeat(object):
    ...
    def make_samples(self, db, verbose=True, cache_path="", RES_model="resnet152", pick_layer="avg"):
        ...
        try:
            print('searching for cache dir in %s' %
                  os.path.join(cache_dir, sample_cache))
            ...
            traceback.print_exc()
            if verbose:
                print("Countings histogram..., config=%s, distance=%s, depth=%s" % (
                    sample_cache, d_type, depth))
