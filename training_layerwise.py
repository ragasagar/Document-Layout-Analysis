...
def do_train(cfg, model, trainiteration,resume=False):
    ...
    if(trainiteration==3):
        checkpointer.save("model_layerwise_final")
    else:
        checkpointer.save("model_fromstep_"+str(trainiteration))
            


def train_layerwise(cfg):
    logger=logging.getLogger(cfg.TRAINING_NAME)
    logger.info('Training Name:'+cfg.TRAINING_NAME)
    ...
    model_prev = cfg.OUTOUT_DIR+"/model_fromstep_1.pth"
    cfg.MODEL.WEIGHTS = model_prev

    logger.info("Training Iteration 2 with Freeze layers at 3 CNN")
    cfg.MODEL.BACKBONE.FREEZE_AT = 3
    model = build_model(cfg)
    logger.info("Model:\n{}".format(model))
    do_train(cfg, model, 2, resume=False) #never resume. Weights are taken from cfg yml

    logger.info("Training Iteration 3 with no freeze layers and very small LR")

    model_prev = cfg.OUTOUT_DIR+"/model_fromstep_2.pth"
    cfg.MODEL.WEIGHTS = model_prev
