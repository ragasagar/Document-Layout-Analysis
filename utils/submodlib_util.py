...
def FL1CMI_wrapper(lake_data, query_data, private_data, eta, image_list, budget=10, metric="cosine", optimizer="LazierThanLazyGreedy", stopIfZeroGain=False,
    ...
    obj = FacilityLocationConditionalMutualInformationFunction(n=lake_data.shape[0],
                                                               ...
                                                               privateData=private_data, metric=metric,
                                                               magnificationEta=eta)
    greedyList = obj.maximize(budget=budget, optimizer=optimizer, stopIfZeroGain=stopIfZeroGain,
                              stopIfNegativeGain=stopIfNegativeGain, verbose=verbose)
    return [x[0] for x in greedyList]
...
def submod_results(lake_data, query_data, eta, targeted_classes, image_list, annotations, image_areas, category_list, budget=10, metric='cosine'):
    ...
    # FL1MI
    FL1MI_results = FL1MI_wrapper(lake_data, query_data, eta, image_list, budget=budget, metric=metric,
                                  optimizer="NaiveGreedy", stopIfZeroGain=False, stopIfNegativeGain=False, verbose=False)

    # FL2MI
    FL2MI_results = FL2MI_wrapper(lake_data, query_data, eta, image_list, budget=10, metric="cosine",
                                  optimizer="NaiveGreedy", stopIfZeroGain=False, stopIfNegativeGain=False, verbose=False)

    # COM
    COM_results = COM_wrapper(lake_data, query_data, eta, image_list, budget=10, metric="cosine",
                              optimizer="LazierThanLazyGreedy", stopIfZeroGain=False, stopIfNegativeGain=False, verbose=False)

    # GCMI
    GCMI_results = GCMI_wrapper(lake_data, query_data, eta, image_list, budget=10, metric="cosine",
                                optimizer="LazierThanLazyGreedy", stopIfZeroGain=False, stopIfNegativeGain=False, verbose=False)

    # LogDetMI
    if eta < 3:
        LogDetMI_results = LogDetMI_wrapper(lake_data, query_data, eta, image_list, budget=10, metric="cosine",
                                            optimizer="LazierThanLazyGreedy", stopIfZeroGain=False, stopIfNegativeGain=False, verbose=False, lambdaVal=1)
